...
class SparseMultiheadSASphereConcat(nn.Module):
    def __init__(self,
                 embed_dim,  # 32
                 num_heads,  # 2
                 indice_key,  # ’sphereformer‘
                 window_size,  # [0.6,0.6,0.6]
                 window_size_sphere,  # [2,2,120]
                 shift_win=False,  # False
                 pe_type='none',  # contextual
                 dropout=0.,  # 0.0
                 qk_scale=None,  # None
                 qkv_bias=True,  # True
                 algo='native',
        ...
        if pe_type == 'contextual':
            ...
            quant_size = kwargs['quant_size']  # [0.025, 0.025, 0.025]
            self.quant_size = to_3d_numpy(quant_size)

            quant_size_sphere = kwargs['quant_size_sphere']  # [1/12, 1/12, 5]
            self.quant_size_sphere = to_3d_numpy(quant_size_sphere)

            self.a = kwargs['a']  # 0.0125
    ...
    def forward(self, sptr_tensor: SparseTrTensor):
        ...
        xyz = sptr_tensor.query_indices[:, 1:]  # <Tensor, [N_s, 3]> voxel内unique剩下来那个点的三维坐标
        batch = sptr_tensor.query_indices[:, 0]  # <Tensor, [N_s,]> 每个voxel的batch id
        ...
        xyz_sphere = cart2sphere(xyz)  # 笛卡尔坐标转球坐标, [N, 3] [theta, beta, r]
