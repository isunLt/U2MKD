...
def grid_sample(pos, batch, size, start, return_p2v=True, return_counts=True, return_unique=False):
    ...
    cluster = voxel_grid(pos, batch, size, start=start)  # grid idx [N, ], 用大小为size的grid对点云pos进行聚类，属于同一个grid的点具有相同的idx
...
def get_indices_params(xyz, batch, window_size, shift_win: bool):
    """
    :param xyz: [N_s, 3] 笛卡尔坐标
    :param batch: [N_s,]
    :param window_size: [0.6, 0.6, 0.6]
    :param shift_win: False
    :return:
    """
