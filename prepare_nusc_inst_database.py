...
from visualize_utils import SemKITTI_label_name_16, visualize_pcd

from tqdm import tqdm

DATA_ROOT = '/data/share/nuscenes/'
SPLIT = 'train'
...
for c_i in THING_LIST:
    INST_DBINFO_PKL[SemKITTI_label_name_16[c_i]] = []

def prepare_file_list():
    nusc = NuScenes_devkit(dataroot=DATA_ROOT, version='v1.0-trainval', verbose=True)
    if SPLIT == "train":
...
def process_one_sequences(info: dict, save_file=True):
    ...
    for thing_id in THING_LIST:
        ...
        thing_name = SemKITTI_label_name_16[thing_id]
        for uq_inst_label in unique_inst_label:  # 对于每一个实例
            ...
            if np.sum(panoptic_label[index]) == 0:
                continue
            if save_file:
                dir_path = os.path.join(DATABASE_SAVE_DIR, SemKITTI_label_name_16[thing_id])
...
if __name__ == '__main__':
    ...
    for pair in tqdm(name_token_pair):
        process_one_sequences(pair, save_file=False)
    for k, v in INST_DBINFO_PKL.items():
        print(f'load {len(v)} {k} database infos')
    with open(INST_DBINFO_PKL_SAVE_PATH, 'wb') as f:
